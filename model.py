...
def zero_pad_column_and_pad_token_rows(smoothed_distribution, gold_token_ids, pad_id):
    ...
    row_padding = gold_token_ids == pad_id
    result[row_padding] = 0
    return result
...
def initialize_adam_optimizer_state(parameter_list):
    ...
    state = {}
    state['m'] = [torch.zeros_like(p) for p in parameter_list]
    state['v'] = [torch.zeros_like(p) for p in parameter_list]
    state['t'] = 0
...
def select_top_k_candidates(candidate_scores, k):
    # Flatten: (num_beams * vocab_size,)
    flat_scores = candidate_scores.reshape(-1)

    # Top-k scores and their flattened indices
...
def append_tokens_to_beam_sequences(beam_sequences, beam_indices, token_ids):
    # Gather the parent beam sequences
    parent_sequences = beam_sequences[beam_indices]

    # Add the new token as the final column
    return torch.cat(
        [parent_sequences, token_ids.unsqueeze(1)],
        dim=1
    )
